...
<html lang="en">
	...
	<body>
		...
		<!-- add feed,add folder pop out here -->
		<div id="add_feed"
			style="height: 0px; width: 0px; position: absolute; z-index: 3; background-color: #fff; display: none;">
			<div class="container">

                            <form class="form-signin"  action="/My_News_Clips/FolderServlet" method="post">
					<h2 class="form-signin-heading">
						Add new feed
					</h2>
                                
                                	<input name="name" type="text" class="input-block-level" placeholder="feed title">

					<input name="url" type="text" class="input-block-level" placeholder="feed url">
                                        
                                        <%
                                            
                                            String username = (String) session.getAttribute("name");
						if (username != null && !username.equals(" ")) {
                                                     ArrayList<FeedFolder> folders = new ArrayList<FeedFolder>();
                                                     int uid = FinalFactory.getUserDAOInstance().findIdByUserName(username);
                                                     folders = FinalFactory.getRssDAOInstance().findFolderByUserID(uid);
                                                     
                                                %>     
                                                
                                        <select name ="folder"  id="category" onchange="getOption(this.value)">
						
                                                
                                                <%   
                                                     if (!folders.isEmpty()) {

									Iterator<FeedFolder> it = folders.iterator();
									int count = 0;
									while (it.hasNext()) {
										FeedFolder ff = it.next();
                                                %>
                                                
                                                <option value="<%=ff.getFeed_folder_id() %>" >
							<%=ff.getName() %>
						</option>
		...
		<div id="add_folder"
			...
			<div class="container">
				...
				<form class="form-signin"  action="/My_News_Clips/FolderServlet" method="post">
					...
					<input name="foldername" type="text" class="input-block-level"
						placeholder="folder name">
					<button name="actiontype" value="addfolder"  class="btn btn btn-primary" type="submit"
						style="float: left; margin-left: 140px; margin-top: 15px; width: 70px;"
						onclick="Lock_CheckForm('add_folder');">
						Confirm
					</button>
					<button name="actiontype" value="cancel"
                                                class="btn btn btn-default"
			...
			<div class="row-fluid">
						...
						<ul class="nav nav-list">
							<li>
								<button class="btn" style="float: right">
									<i class="icon-folder-close" onclick="locking('add_folder');"></i>
								</button>
								<button class="btn" style="float: right">
									<i class="icon-plus" onclick="locking('add_feed');"></i>
								</button>
							</li>
							...
							<%
								...
								ArrayList<FeedFolder> feedfolder = new ArrayList<FeedFolder>();

								feedfolder = FinalFactory.getRssDAOInstance().findFolderByUserID(uid);
							%>

							<!-- rss menu -->

							<%
								if (!feedfolder.isEmpty()) {

									Iterator<FeedFolder> it = feedfolder.iterator();
									int count = 0;
									while (it.hasNext()) {
										FeedFolder ff = it.next();
										count++;
							...
							<ul id="<%=count%>" style="display: none">
								...
								<%
									...
									feedlist = FinalFactory.getRssDAOInstance().findFeedFromUidFolderID(uid,ff.getFeed_folder_id());

									Iterator<Feed> itfeeds = feedlist.iterator();

									while (itfeeds.hasNext()) {
										Feed feed = itfeeds.next();

								%>

								<li>
									<a href="RSS_reader.jsp?url=<%=feed.getUrl()%>"><%=feed.getFtitle()%></a>
								</li>
							...
							<%
                                                        ...
                                                        <li style="font-size:18px;">feeds</li>
				...
				<div class="span9">
					...
					<%
						...
						System.out.println("url in rss reader is:" + url);
					%>

					<% if(session.getAttribute("name")!=null && ""!=session.getAttribute("name") && url!=null){ %>
                                            <jsp:include page="FeedEntryList.jsp" flush="true">
                                                <jsp:param name="url" value="<%=url%>" />
                                            </jsp:include>
                                        <%}else if(session.getAttribute("name")!=null && ""!=session.getAttribute("name") && url==null){%>
                                        
                                        <div class="hero-unit" style="height:150px;">
                                        Welcome to RSS reader!
                                        <br>
                                        Please click on a feed to read content!
                                        </div>
